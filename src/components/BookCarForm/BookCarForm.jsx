...
const validationSchema = Yup.object().shape({
  email: Yup.string()
    ...
    .test("Invalid email domain", (value) => {
      ...
      return (
        parts[1].includes(".") && parts[1].split(".").filter(Boolean).length > 1
      );
    }),
  name: Yup.string()
...
const BookCarForm = ({ id }) => {
  const handleSubmit = (values, { resetForm }) => {
    values.id = id;
    toast.success(
      `${values.name}, your rent confirmed! We will contact you soon.`,
      toastStyles
    );
    resetForm();
  };
    ...
    <div className={css.wrapper}>
      ...
      <Formik
        ...
        <Form className={css.form}>
          ...
          <MyDatePicker calendarClassName={css.calendarWrp} name="date" />
